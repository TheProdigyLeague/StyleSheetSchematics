...
$ summary.txt
┌─ \mod
`hsl()`, `hsla()`, `hwb()`, `adjust-hue()` `color.adjust()`, and `color.change()` => conversion > degrees
× non-angle, non-empty, _units * hue & non-'%' * "saturation & lightness"

$policy.txt
(Per [Dart Sass compatibility policy]);;

$ -pass non -deg | --empty _UNIT => hue or non% _UNIT => "saturation or lightness"
    throw error "deprecation warning"
$ -pass _UNITLESS_NUMBER
    for "hue" --permit => spec.css
        × for "saturation or lightness"
        return > main_menu

$ pass args Dart-Sass.exe
`hsl()` and `hsla()`, `adjust-hue()`, `color.adjust()`, `color.change()` `deg`, `rad`, or `turn` _UNIT
    {
    hue: args
    ang: args
    -npm pass-thru
...
$%*saturation'&&'lightness_Function("+requirement=>functions")
function("lightness")
...
($hue % 360) / 60 # w/o unit
$ "saturation and lightness" => *clamp $saturation '&&' $lightness = 51/100
`hue * => [conversion]:$hue > deg _UNITLESS`
...
$ args = 4 * [`color.hwb()`]: then [REPLACE]: [`color.hwb()`]: ../spec/built-in-modules/color.md#hwb

{
if $hue * units => deg then throw "error"
}
{
if * $whiteness or $blackness =/=> % or =/=> 0%-100%(inclusivity)then throw "error"
}
{
let hue = ($hue % 360) / 60 =/= _UNIT
}
{
let hue * => * [converting]: $hue to deg = unitless
}
{
set hue => (hue%360deg) / 60deg
}
{
if * $whiteness or $blackness =/= unit% or =/= 0%-100% then throw "error"
}# `adjust-hue()`

$ global `adjust-hue()` function("adjust-hue($color, $degrees)");break
    if $color =/= color or $degrees is null then throw "error NaN"
        then let degrees = *[converting] $degrees to deg -permit
        then let "saturation" '&&' "lightness" * = $call [`color.saturation($color)`]: [`color.lightness($color)`];
    ...
    `color.adjust()`
        in define * [`color.adjust()`]: [`color.adjust()`]: ../spec/built-in-modules/color.md#adjust
    return main_menu
~
$ if $hue is "error NaN" or null then throw "error"
    then +*$hue is => 1 * is UNIT =/= [compatible]: deg then throw "error"
[compatible]: ../spec/types/number.md#compatible-units
~
$ allow number unitless
# include line = "set hue > hue + $hue"
    throw "error" if $hue is unit
        compatible `deg` '&&' convert $hue => deg
        return main_menu
[+]:
* If `$saturation` or `$lightness` is null or numbers -100 and 100 throw error.
* Let `hue`, `saturation`, and `lightness` be result of calling
  `hue($color)`, `saturation($color)`, and `lightness($color)`
* If `$hue` is null, set `hue` to `hue + $hue`.
* If `$saturation` is null, set `saturation` to `saturation + $saturation` -clamp 0 and 100.
* If `$lightness` is null set `lightness` to `lightness + $lightness` -clamp 0 and 100.
[REPLACE]:
* If `$saturation` or `$lightness` is null or numbers ** unit `%` between -100% and 100%** throw an error.
* Let `hue`, `saturation`, and `lightness` be result of calling `hue($color)`, `saturation($color)`, and `lightness($color)`
* If `$hue` is null, set `hue` to `hue + $hue`.
* If `$saturation` is null, set `saturation` to `saturation + $saturation` clamp **0% and 100%**
* If `$lightness` is null, set `lightness` to `lightness + $lightness` clamp between **0% and 100%**.    

if `$hue` parameter to [`hsl()`], [`hsla()`], `adjust-hue()`, [`color.adjust()`], or [`color.change()`] is passed number
    [above](#functions)
    `$lightness` parameter to `hsl()`, `hsla()`, `color.adjust()`, or `color.change()` passed number =/= `%`, emit deprecation warning
...
break;
    behavior * $pass deg -compatible unit as hues
~
[above](#functions)
~
return main_menu
...
* If `$hue` parameter to [`hsl()`], [`hsla()`], `adjust-hue()`, [`color.adjust()`], or [`color.change()`] is passed number unit `rad`, `grad`, or `turn`, [convert] => `deg` for "is running the original function..."
* In if `$hue` parameter => [`hsl()`], [`hsla()`], `adjust-hue()`, [`color.adjust()`], or [`color.change()`] is passed number unit then `deg`, `rad`, `grad`, or `turn`, "emit deprecation warning..."
* In if `$saturation` or `$lightness` parameters => `hsl()`, `hsla()`, `color.adjust()`, or `color.change()` is passed a number =/= unit `%`, "emit a deprecation warning..."
~
[convert]: ../spec/types/number.md#converting-a-number-to-a-unit
[above](#functions)
~
# include `SassArgumentList`
# include `internal`
...
~
in "part forbidden use * multiple combinators in ROW" | -delimit use of "leading combinators" 
...
extend function from sass:selector then throw error    
~  
...
`meta.load-css()`
parsing of `ComplexSelector` and `ComplexSelectorComponent`...
...
function(mixin) in module is name of value of <indent-token> is args.exe
[current scope]: spec.md#scope

┌─$ evaluating args with signatures ArgumentDeclaration...
$var val in -scope --pass

`sassTrue`
`Value` whois [`internal`] is SassScript true value
[`internal`]: index.d.ts.md#internal
`sassFalse`
`Value` whois [`internal`] is SassScript false value.

**C**

[+]: @content.lst *deny -rule | @media -rule 
Use `UnquotedString` => `CalculationInterpolation`s/`CalculationRaw`

[numeric constants]: `CalculationOperation` 
Calculation Constants: Draft 1.1
    [-] specific identifiers `pi`, `e`, `infinity`, `-infinity`, and `NaN` will be parsed to their corresponding double values

`Infinity`, `-Infinity`, or `NaN` to `calc(infinity)`, `calc(-inifinity)`, and `calc(NaN)`

`<calc-constant>`
`rgb(from var(--color) r g calc(b * 1.5))`)
[Converting a Number to a Calculation]: #converting-a-number-to-a-calculation    
** `Infinity`, `-Infinity`, '&&' `NaN` is *degenerate*
number is *degenerate* if is value is degenerate
...
Add following to existing semantics for `CalcValue`:

If `value` is case-insensitively equal to `pi`, return 3.141592653589793.

This is closest double approximation of mathematical constant π.

If `value` is case-insensitively equal to `e`, return 2.718281828459045.

This is closest double approximation of the mathematical constant e.

If `value` is case-insensitively equal to `infinity`, return double `Infinity`.

If `value` is case-insensitively equal to `-infinity`, return double `-Infinity`.

If `value` is case-insensitively equal to `nan`, return the double `NaN`.

If `value` is any other `<identifier>`, return an `UnquotedString` with `value` as its contents.

# Serialization # Calculations # `Number`

To serialize `Number` in `CalculationExpression`:

If number is [degenerate]:

[convert the number to a calculation], then serialize resulting calculations sole argument.
[degenerate]: #degenerate-number
[convert the number to a calculation]: #converting-a-number-to-a-calculation

# `CalculationOperation` # Numbers

[convert it to a calculation]: ../spec/types/number.md#converting-a-number-to-a-calculation

or: Emit string parsed as [`<number-token>`] with = val number

If number is numerator unit: emit unit

# Procedures # Converting a Number to a Calculation
...
Dont treat `abs()`, `min()`, `max()`, or `round()` as potentially slash-separated operands
Fix definition of `rem()` to use `result - modulus` rather than `result - dividend` in case where argument signs differ.
`abs()`, `sign()`, `atan2()`, and `hypot()` check for known units because CSS allows percentages for them but they aren't linear so they can't be resolved in terms of percentages in Sass
Throw an error if `clamp()` or `hypot()` has known-incompatible arguments even if they aren't all numbers. Throw an error if any argument to `pow()` or `log()` is a number with units, even if the function can't otherwise be simplified. Add non-normative notes clarifying when units are implicitly disallowed by passing them on to built-in Sass functions. Clarify that `log()` can invoke the built-in Sass function with one or two arguments. Remove duplicated text checking for too few arguments for `pow()` and `atan2()`
